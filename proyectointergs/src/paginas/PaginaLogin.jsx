...
/* Estilos muy sencillos (Tailwind) */
export default function PaginaLogin() {
  const { usuario, rol,
          loginGoogle, loginCorreo, registrar, olvidarPass } = useAuth();
  ...
  const submit = async(e)=>{
    ...
    try {
      if (esRegistro) await registrar(form.email, form.pass, form.nombre);
      else            await loginCorreo(form.email, form.pass);
    } catch (e) { setErr(e.message); }
  };

  const resetPass = async()=>{
    if (!form.email) return setErr("Escribe tu correo arriba primero");
    try { await olvidarPass(form.email);
